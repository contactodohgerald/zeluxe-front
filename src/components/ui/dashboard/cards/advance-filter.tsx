import { Button, Card } from 'antd';
import { Form, Input } from '@/components/ui/form';

export const AdvanceFilter = () => {
  return (
    <Card className="rounded-[1.4rem] bg-[#f9f9f9] p-[1.3rem] shadow-filter">
      <div className="mb-2">
        <h3 className="text-center font-lato text-[1.3rem] font-bold leading-[1.53rem] tracking-[0.03em] text-primary">
          Advanced Filter Options
        </h3>
      </div>
      <Form onSubmit={() => {}} schema={{} as any}>
        {({ register }) => (
          <>
            <Input
              className="h-[3.2rem] w-full rounded-md bg-white px-3 py-3 font-lato text-[0.96rem] font-bold leading-[1.2rem] tracking-[0.03em] text-secondary outline-none focus:outline-none"
              type="text"
              label="Location"
              // error={formState.errors['email']}
              registration={register('email')}
            />
            <Input
              className="h-[3.2rem] w-full rounded-md bg-white px-3 py-3 font-lato text-[0.96rem] font-bold leading-[1.2rem] tracking-[0.03em] text-secondary outline-none focus:outline-none"
              type="text"
              label="Category"
              // error={formState.errors['email']}
              registration={register('email')}
            />
            <div className="flex gap-x-[2.88rem]">
              <Input
                className="h-[3.2rem] w-full rounded-md bg-white px-3 py-3 font-lato text-[0.96rem] font-bold leading-[1.2rem] tracking-[0.03em] text-secondary outline-none focus:outline-none"
                type="text"
                label="Type"
                // error={formState.errors['email']}
                registration={register('email')}
              />
              <Input
                className="h-[3.2rem] w-full rounded-md bg-white px-3 py-3 font-lato text-[0.96rem] font-bold leading-[1.2rem] tracking-[0.03em] text-secondary outline-none focus:outline-none"
                type="text"
                label="Bedrooms"
                // error={formState.errors['email']}
                registration={register('email')}
              />
            </div>
            <div className="flex gap-x-[2.88rem]">
              <Input
                className="h-[3.2rem] w-full rounded-md bg-white px-3 py-3 font-lato text-[0.96rem] font-bold leading-[1.2rem] tracking-[0.03em] text-secondary outline-none focus:outline-none"
                type="text"
                label="Min Price"
                // error={formState.errors['email']}
                registration={register('email')}
              />
              <Input
                className="h-[3.2rem] w-full rounded-md bg-white px-3 py-3 font-lato text-[0.96rem] font-bold leading-[1.2rem] tracking-[0.03em] text-secondary outline-none focus:outline-none"
                type="text"
                label="Max Price"
                // error={formState.errors['email']}
                registration={register('email')}
              />
            </div>
            <Input
              className="h-[3.2rem] w-full rounded-md bg-white px-3 py-3 font-lato text-[0.96rem] font-bold leading-[1.2rem] tracking-[0.03em] text-secondary outline-none focus:outline-none"
              type="text"
              label="Keywords"
              placeholder=""
              // error={formState.errors['email']}
              registration={register('email')}
            />
            <Button className="block h-[2.68rem] w-full rounded-[0.4rem] bg-primary py-2 font-lato text-[0.96rem] font-bold leading-[1.2rem] tracking-[0.03em] text-white outline-none focus:outline-none data-[active]:bg-primary data-[hover]:bg-primary">
              Search
            </Button>
          </>
        )}
      </Form>
    </Card>
  );
};
