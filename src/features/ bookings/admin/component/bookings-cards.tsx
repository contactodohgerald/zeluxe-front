...
export const BookingsCards = ({ bookings }: { bookings: RentalBooking[] }) => {
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {bookings?.map((booking) => (
        <div
          className="relative mt-8 w-full max-w-sm shadow-md"
          key={booking?.id}
        >
          <Link
            to={paths.app.admin.booking.getHref(booking?.id)}
            className="relative inline-block w-full transform transition-transform duration-300 ease-in-out hover:text-primary"
          >
            <div className="rounded-lg">
              <div className="relative flex h-60 justify-center overflow-hidden rounded-t-lg">
                ...
                <span
                  className={`absolute left-0 top-0 z-10 ml-3 mt-3 inline-flex select-none rounded-lg ${booking.status === 'confirmed' ? 'bg-success' : 'bg-grey-5'} px-3 py-2 text-sm font-medium text-white`}
                >
                  {' '}
                  {booking?.status}{' '}
                </span>
                ...
                <div className="mt-4 grid grid-cols-2">
                  <div className="flex items-center">
                    <div className="relative">
                      ...
                      >
                        {booking?.rental?.address?.state.name},{' '}
                        {booking?.rental?.address?.state.name.slice(0, 1) +
                          booking?.rental?.address?.state.name.slice(1, 2)}{' '}
                        {booking?.rental?.address?.zip_code},{' '}
                        {booking?.rental?.address?.country.name}
                      </p>
                    </div>
                  </div>

                  <div className="flex items-center justify-end">
                    <p className="inline-block whitespace-nowrap rounded-xl font-semibold leading-tight text-primary">
                      <span className="text-sm uppercase"> {currencyNGN} </span>
                      <span className="text-lg">
                        {parseInt(booking?.amount)}
                      </span>
                      {/* /m */}
                    </p>
                  </div>
